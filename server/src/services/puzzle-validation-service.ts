...
function evaluatePatternMatch(seed: number, difficulty: number, submission: PatternMatchSubmission) {
  ...
  for (let index = 0; index < submission.answers.length; index += 1) {
    ...
    if (!round) break;
    if (submission.answers[index] !== round.correctOption) break;
    correct += 1;
  }

  return clampProgress((correct / rounds.length) * 100);
}

function evaluateTileSlide(seed: number, difficulty: number, submission: TileSlideSubmission) {
  const initial = buildTilePuzzle(seed, difficulty).tiles;
  if (submission.tiles.length !== initial.length) return 0;

  const correct = submission.tiles.filter((value, index) => {
...
export function evaluatePuzzleSubmission(
  ...
  switch (submission.kind) {
    ...
    case "pattern_match":
      return evaluatePatternMatch(seed, difficulty, submission);
    case "word_scramble": {
    ...
    case "tile_slide":
      return evaluateTileSlide(seed, difficulty, submission);
    case "sudoku_mini": {
